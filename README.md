...
Markdown file on disk. Designed to slot into a personal Nextcloud / VPS stack
...
PDFs you can actually read at full size, wikilinks between notes, tags, and
mirroring everything 1:1 into a Nextcloud folder.
...
middle ground: **the on-disk format stays plain Markdown** so AI tools like
Claude Desktop can read your notes directly, while the editor feels like a
modern block-based notes app rather than a code editor.
...
- Inline `[[wikilinks]]` with autocomplete, click to navigate,
  `Cmd+Click` for new tab
- Inline `#tags` with autocomplete (`#` trigger), grouped tag panel in sidebar
...
- Folder tree in the sidebar with unlimited nesting, drag-free reorder via
  rename/move
...
- Create / rename / soft-delete notes and folders from the sidebar
...
- YAML frontmatter (`created`, `modified`) is auto-managed on save
...
```
notes-app/
├── web/                    # Vite + React app (the only thing that runs)
│   ├── src/
│   │   ├── App.tsx              # main shell (sidebar, editor, modals)
│   │   ├── pdfBlock.tsx         # custom PDF block + handwrite block schema
│   │   ├── handwriteBlock.tsx   # rendering + re-edit hook
│   │   ├── handwriteModal.tsx   # full-screen canvas editor
│   │   ├── wikilinkInline.tsx   # [[wikilink]] inline content
...
- **Mobile-native iOS app** — the PWA on iPad is the iOS story; works well
  with Apple Pencil because PointerEvents expose pressure to the browser
...
- [ ] Note templates
- [ ] Export single note as PDF / standalone HTML
- [ ] Code-block syntax highlighting (Shiki or similar)
- [ ] Drag-drop to reorder folder tree
